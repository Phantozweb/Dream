...
// These components are defined later in this file
// and will be exported at the end
...
const SidebarContextOriginal = React.createContext<SidebarContextOriginal | null>(null)

function useSidebarOriginal() {
  const context = React.useContext(SidebarContextOriginal)
  if (!context) {
...
>(({ defaultOpen = true, open: openProp, onOpenChange: setOpenProp, className, style, children, ...props }, ref) => {
  ...
  const contextValue = React.useMemo<SidebarContextOriginal>(
    () => ({
      state,
      open,
      setOpen,
  ...
  return (
    <SidebarContextOriginal.Provider value={contextValue}>
    ...
    </SidebarContextOriginal.Provider>
  )
})
...
export function Sidebar({ children, className }: { children: React.ReactNode; className?: string }) {
  ...
  return (
    <div
      className={cn("h-screen border-r bg-sidebar transition-all duration-300", expanded ? "w-64" : "w-16", className)}
    >
      {children}
    </div>
  )
...
>(({ side = "left", variant = "sidebar", collapsible = "offcanvas", className, children, ...props }, ref) => {
  const { isMobile, state, openMobile, setOpenMobile } = useSidebarOriginal()
...
// Create the SidebarHeader component
...
// Create the SidebarContent component
...
// Create the SidebarFooter component
...
// Create the SidebarSeparator component
...
// Create the SidebarGroup component
...
// Create the SidebarGroupLabel component
export function SidebarGroupLabel({ children, className }: { children: React.ReactNode; className?: string }) {
  const { expanded } = useSidebar()
  if (!expanded) return null
  return <div className={cn("px-4 py-1 text-xs text-sidebar-foreground/60 font-medium", className)}>{children}</div>
}
...
// Create the SidebarGroupContent component
...
// Create the SidebarMenu component
...
// Create the SidebarMenuItem component
...
// Create the SidebarMenuButton component
export function SidebarMenuButton({
  ...
  if (asChild) {
    // When asChild is true, we need to clone the child element and pass our props to it
    // instead of trying to pass them to React.Fragment
    return (
      <>
        {React.Children.map(children, (child) => {
          if (React.isValidElement(child)) {
            return React.cloneElement(child, {
              ...
              children: typeof children === "function" ? children({ expanded }) : child.props.children,
            })
          }
          return child
        })}
  ...
  // When not using asChild, we can use a regular button
  return (
    <button
      className={cn(
        "flex items-center w-full rounded-md px-3 py-2 text-sm font-medium",
...
>(({ asChild = false, isActive = false, variant = "default", size = "default", tooltip, className, ...props }, ref) => {
  const Comp = asChild ? Slot : "button"
  const { isMobile, state } = useSidebarOriginal()
...
export {
  SidebarGroupActionOriginal as SidebarGroupAction,
  SidebarInput,
  SidebarInset,
  SidebarMenuActionOriginal as SidebarMenuAction,
  SidebarMenuBadgeOriginal as SidebarMenuBadge,
  SidebarMenuSkeletonOriginal as SidebarMenuSkeleton,
  SidebarMenuSubOriginal as SidebarMenuSub,
  SidebarMenuSubButtonOriginal as SidebarMenuSubButton,
  SidebarMenuSubItemOriginal as SidebarMenuSubItem,
  SidebarRail,
  SidebarTrigger,
}
